...
- **カラムをまたぐ選択・ページをまたぐ選択にも対応**（Prototype 2.4）。同一段落（PDF.js/PyMuPDF
  ...
  設計判断の詳細は`docs/design-notes.md`「Prototype 2.4B」を参照。
- 既知の制約（Prototype 2.4時点）: 数式番号付近での選択が不安定な場合がある（詳細は
  `docs/design-notes.md`）。pure scan PDF（テキストレイヤーがないPDFはエラー表示。テキストレイヤーは
  あるが品質が低いPDFについては、下記のOCR fallbackで手動の読み直しができる）は今回のPyMuPDF
  layout解決の対象外（既存のOCR fallback領域のまま）。解析履歴・注釈の永続保存は未実装。
