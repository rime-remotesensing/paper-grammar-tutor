...
> **Archived development document:** この文書は Prototype 2.5 時点の記録です。現在の導入手順は [README](../../README.md) と [Getting Started](../GETTING_STARTED.md) を参照してください。
...
ollama pull qwen2.5:14b-instruct   # 任意。7Bより一貫して良いわけではない（../design-notes.md参照）
...
- 対応: ローカルPDFファイルの読み込み・連続縦スクロール表示・zoom in/out・テキストレイヤーからの
...
- **カラムをまたぐ選択・ページをまたぐ選択にも対応**（Prototype 2.4）。同一段落（PDF.js/PyMuPDF
  双方が認識する同一text block）内なら通常のブラウザ選択をそのまま使い、異なる段落・異なるカラム・
  異なるページにまたがる選択は、ローカルで別途起動する`services/pymupdf_layout/`（PyMuPDF）が
  段落構造を解決して読み順どおりに再構成する。このサービスが起動していない場合、同一段落内の選択は
  従来どおり動作するが、段落・カラム・ページをまたぐ選択は明示的なエラーメッセージを表示する
  （誤った文字列を解析対象へ渡さないため）。起動方法は`services/pymupdf_layout/README.md`参照。
  設計判断の詳細は`../design-notes.md`「Prototype 2.4B」を参照。
- **数式に隣接する選択への対応**（Prototype 2.5B〜G）。PDF内に視覚的には存在するがPDF.js/PyMuPDF
  どちらもテキストとして抽出できないインライン数式記号（斜体変数など、実例: k, e, θ）を含む選択は、
  ローカルのPaddle OCRサービスが起動していれば当該記号だけを画像認識で補完し、それ以外の本文は
  PyMuPDF/PDF.js抽出テキストのまま扱う（OCR結果で本文全体を置き換えることはない）。Paddleが未起動、
  または信頼できる補完ができない場合は、その選択のみ明示的なエラーで停止する（誤った文字列を解析対象
  へ渡さないため）。通常の英文選択（Paddle不要）には影響しない。
- **本文から数式番号（例:「(5)」）へ続く選択への対応**（Prototype 2.5G）。本文が採番済みの
  display equationへ向かって終わる選択は、その数式の内容を読み取ろうとする代わりに`[式 (5)]`という
  placeholderとして本文に挿入する（数式のOCR・LaTeX化・内容の推測は一切行わない）。解析（骨格を見る）
  へ送る際は内部的に`[EQUATION_5]`という表現へ自動変換され、数式が独立した節として誤解析されない
  ようプロンプトに最小限のルールを追加している。数式番号だけを選択した場合は本文を自動で付け足さない。
  数式から本文へ向かう逆方向のドラッグ等、まだ検証していない範囲は明示的なエラーで停止する。
- **引用番号（例:「[9]」）は解析から除外**（Prototype 2.5H）。テキスト欄・PDF上の表示はそのまま
  （引用番号は消えない）だが、「解析」を押した際に送信される文からは引用番号（範囲・列挙を含む、
  例:「[1]–[3], [9], [11]」）のみ確実に除去してから解析する。引用は文法上の要素を持たないため、
  骨格・Structure Tree・ReadingGuideのどこにも独立したノードとして現れない。数式placeholder
  （`[式 (N)]`/`[EQUATION_N]`）とは判定基準が異なり誤って除去されることはない。設計判断の詳細は
  `../design-notes.md`「Prototype 2.5A〜H」を参照。
- 既知の制約（Prototype 2.5H時点）: display equation本体（数式の内容そのもの）の読み取り・LaTeX化は
  非対応（`[式 (N)]`はplaceholderであり内容を表さない）。pure scan PDF（テキストレイヤーがないPDFは
  エラー表示。テキストレイヤーはあるが品質が低いPDFについては、下記のOCR fallbackで手動の読み直しが
  できる）は今回のPyMuPDF layout解決の対象外（既存のOCR fallback領域のまま）。解析履歴・注釈の永続
  保存は未実装。
...
- 方式は **page-wide OCR + spatial extraction**（フィージビリティ検証はPrototype
  1.2A〜1.2Cを参照、`../design-notes.md`）。選択領域だけを直接OCRする方式は精度が
...
Prototype 0.2のholdout評価（未使用の57文、詳細は`../design-notes.md`）で判明した、現時点の
文法解析エンジンの既知の限界:
